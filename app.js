  ...
  function Home(props) {
    ...
    return h('div', { className: 'dashboard' },
      ...
      h('section', { className: 'balance-hero' },
        h('div', { className: 'hero-label' }, 'Tài sản ròng'),
        h('div', { className: 'hero-balance' }, D.fmtVND(d.tot.netWorth), h('span', null, ' đ')),
        h('div', { className: 'hero-foot' },
          h('span', null, 'Tổng tài sản ', h('b', null, D.fmtShort(d.tot.assets))),
          h('span', null, d.tot.liability ? 'Nợ phải trả ' + D.fmtShort(d.tot.liability) : 'Không có dư nợ'))),
      ...
      h('section', { className: 'panel liquidity-panel' },
        h('div', { className: 'panel-head' },
          h('div', null,
            h('h2', { className: 'panel-title' }, 'Thanh khoản'),
            h('div', { className: 'panel-sub' }, 'Tiền có thể sử dụng ngay và vị thế dự phóng 30 ngày')),
        ...
        h('div', { className: 'metrics two', style: { marginBottom: 0 } },
          h('div', { className: 'metric', style: { boxShadow: 'none', background: 'var(--surface-2)' } },
            h('div', { className: 'metric-label' }, 'Tiền khả dụng'),
            h('div', { className: 'metric-value' }, D.fmtShort(d.tot.liquid)),
            h('div', { className: 'metric-foot' }, 'Số dư liquid hiện tại')),
          h('div', { className: 'metric', style: { boxShadow: 'none', background: 'var(--surface-2)' } },
            h('div', { className: 'metric-label' }, 'Sau 30 ngày'),
            h('div', { className: cx('metric-value', next30Net >= 0 ? 'pos' : 'neg') }, D.fmtShort(forecast30.end)),
            h('div', { className: 'metric-foot' },
              (next30Net >= 0 ? '+' : '−') + D.fmtShort(Math.abs(next30Net)) + ' so với hôm nay')))),

      h('section', { className: 'dashboard-grid wide' },
        h('div', { className: 'panel chart-panel' },
          h('div', { className: 'panel-head' },
            h('div', null, h('h2', { className: 'panel-title' }, 'Tiền khả dụng — 30 ngày qua'), h('div', { className: 'panel-sub' }, 'Diễn biến tiền mặt thực tế')),
            h('button', { className: 'text-link', onClick: function () { props.onGo('forecast'); } }, 'Dự phóng →')),
