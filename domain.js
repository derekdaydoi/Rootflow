...
(function (global) {
  ...
  /* Repayment decomposition. borrowingCost chỉ là field tương thích ngược;
     primitive chuẩn là interestAmount + feeAmount. */
  ...
  function repayCost(f) {
    ...
    if (f.interestAmount !== undefined || f.feeAmount !== undefined) {
      return repayInterest(f) + repayFee(f);
    }
    return Math.abs(Number(f.borrowingCost) || 0);
  }

  function repayInterest(f) {
    if (!f || f.kind !== 'repay') return 0;
    if (f.interestAmount !== undefined && f.interestAmount !== null && f.interestAmount !== '') {
      return Math.abs(Number(f.interestAmount) || 0);
    }
    return f.feeAmount === undefined ? Math.abs(Number(f.borrowingCost) || 0) : 0;
  }

  function repayFee(f) {
    if (!f || f.kind !== 'repay') return 0;
    return Math.abs(Number(f.feeAmount) || 0);
  }
  ...
  function collectTotal(f) {
    if (!f || f.kind !== 'collect') return Math.abs(Number(f && f.amount) || 0);
    if (f.principalAmount !== undefined || f.interestAmount !== undefined || f.feeAmount !== undefined) {
      return collectPrincipal(f) + collectInterest(f) + collectFee(f);
    }
    return Math.abs(Number(f.amount) || 0);
  }
  ...
  function contractPaymentTotals(contract, flows, opts) {
    opts = opts || {};
    var totals = { principal: 0, interest: 0, fee: 0 };
    ...
    live(flows || []).forEach(function (f) {
      ...
      if (opts.afterDate && String(f.date || '') <= opts.afterDate) return;
      totals.principal += kind === 'repay' ? repayPrincipal(f) : collectPrincipal(f);
      totals.interest += kind === 'repay' ? repayInterest(f) : collectInterest(f);
      totals.fee += kind === 'repay' ? repayFee(f) : collectFee(f);
    });
    return totals;
  }
  ...
  function contractOutstandingPrincipal(contract, flows, opts) {
    ...
    var hasSnapshot = contract && contract.currentOutstanding !== undefined && contract.currentOutstanding !== null;
    if (hasSnapshot && contract.outstandingAsOf && !opts.afterDate) opts.afterDate = contract.outstandingAsOf;
    var paid = contractPaymentTotals(contract, flows, opts).principal;
    var base = hasSnapshot ? Number(contract.currentOutstanding) || 0 : Number(contract && contract.originalPrincipal) || 0;
  ...
  function contractSchedule(contract) {
    ...
    var principal = Math.max(0, contract.currentOutstanding !== undefined && contract.currentOutstanding !== null
      ? Number(contract.currentOutstanding) || 0 : Number(contract.originalPrincipal) || 0);
    if (!(principal > 0)) return [];
    var start = /^\d{4}-\d{2}-\d{2}$/.test(String(contract.startDate || '')) ? contract.startDate : today();
    var end = /^\d{4}-\d{2}-\d{2}$/.test(String(contract.maturityDate || '')) ? contract.maturityDate : null;
    if (!end) return [];
    var frequency = contract.interestFrequency === 'at_maturity' ? 'at_maturity' : 'monthly';
    var dates = [];
    if (frequency === 'at_maturity') dates.push(end);
    else {
      var first = /^\d{4}-\d{2}-\d{2}$/.test(String(contract.firstPaymentDate || '')) ? contract.firstPaymentDate : null;
      if (!first) return [];
      for (var i = 0; i < 600; i++) {
    ...
    return dates.map(function (date, index) {
      ...
      var fee = contract.feeFrequency === 'per_period' ? contractFee
        : contract.feeFrequency === 'one_time' && !contract.feePaid && contract.feeDueDate === date ? contractFee : 0;
      return {
        index: index, date: date, principalAmount: principalAmount, interestAmount: interest, feeAmount: fee,
        amount: principalAmount + interest + fee,
  ...
  function contractFeeTotal(contract) {
    return contractSchedule(contract).reduce(function (sum, row) { return sum + row.feeAmount; }, 0);
  }
  ...
  var CONFIDENCE = {
    ...
    INFERRED: { label: 'Suy luận', short: 'Inferred' },
    UNKNOWN: { label: 'Chưa biết', short: 'Unknown' },
  ...
  function monthSummary(accounts, flows, ym) {
    ...
    var r = {
      ...
      debtService: 0, debtPrincipal: 0, borrowingCost: 0, interestExpense: 0, fees: 0, unallocatedDebtPayment: 0,
    ...
    for (var i = 0; i < inMonth.length; i++) {
      ...
      if (isLegacyDebtPayment(f)) {
        ...
        var interestExpense = repayInterest(f);
        var feeExpense = repayFee(f);
  ...
  /* Control metrics không thay đổi ledger. Rollover rate là giả định quản trị,
     tách khỏi điều khoản hợp đồng và chỉ áp vào revolving principal. */
  function controlMetrics(data, opts) {
    data = data || {};
    opts = opts || {};
    var baseDate = opts.baseDate || today();
    var to = addDays(baseDate, 30);
    var accounts = data.accounts || [], flows = data.flows || [], contracts = data.contracts || [];
    var bal = balances(accounts, flows);
    var total = totals(accounts, bal);
    var revolving = 0, installment = 0;
    accounts.forEach(function (account) {
      if (!account || account.archived || account.type !== 'credit_card') return;
      var current = Math.max(0, Number(bal[account.id]) || 0);
      var installmentPart = Math.max(0, Number(account.installmentBalance) || 0);
      var revolvingPart = account.revolvingBalance === null || account.revolvingBalance === undefined
        ? Math.max(0, current - installmentPart) : Math.max(0, Number(account.revolvingBalance) || 0);
      installment += Math.min(current, installmentPart);
      revolving += Math.min(current, revolvingPart);
    });
    var monthlyDebtService = 0, monthlyInterest = 0, monthlyFees = 0, unallocatedFees = 0, lendingInterest = 0, plannedExpenses = 0;
    live(flows).forEach(function (flow) {
      if (!flow || flow.skipped || flow.confirmed || !flow.date || flow.date < baseDate || flow.date > to) return;
      if (flow.kind === 'repay') {
        monthlyDebtService += repayTotal(flow);
        monthlyInterest += repayInterest(flow);
        monthlyFees += repayFee(flow);
      } else if (flow.kind === 'interest_out') {
        var interestOut = Math.abs(Number(flow.amount) || 0);
        monthlyDebtService += interestOut;
        monthlyInterest += interestOut;
      } else if (flow.kind === 'fee') {
        monthlyFees += Math.abs(Number(flow.amount) || 0);
      } else if (flow.kind === 'collect') lendingInterest += collectInterest(flow);
      else if (flow.kind === 'interest_in') lendingInterest += Math.abs(Number(flow.amount) || 0);
      else if (flow.kind === 'expense') plannedExpenses += Math.abs(Number(flow.amount) || 0);
    });
    contracts.forEach(function (contract) {
      if (!contract || contract.type !== 'payable' || contract.status === 'closed') return;
      if (contract.feeFrequency === 'one_time' && !contract.feePaid && !contract.feeDueDate) unallocatedFees += Math.max(0, Number(contract.feeAmount) || 0);
      if (contractSchedule(contract).length) return;
      var normalizedPayment = Math.max(0, Number(contract.monthlyPayment) || 0);
      var normalizedInterest = Math.max(0, Number(contract.monthlyInterestAmount) || 0);
      if (!normalizedInterest && contract.actualInterestMethod === 'flat') {
        normalizedInterest = Math.round(Math.max(0, Number(contract.originalPrincipal) || 0) * Math.max(0, Number(contract.interestRate) || 0) / 100);
      }
      if (!normalizedPayment) normalizedPayment = Math.max(0, Number(contract.monthlyPrincipal) || 0) + normalizedInterest;
      var normalizedFee = contract.feeFrequency === 'per_period' ? Math.max(0, Number(contract.feeAmount) || 0) : 0;
      monthlyDebtService += normalizedPayment + normalizedFee;
      monthlyInterest += normalizedInterest;
      monthlyFees += normalizedFee;
    });
    var expectedIncome = (data.recurringIncomes || []).reduce(function (sum, income) {
      if (!income || income.archived || income.frequency !== 'monthly') return sum;
      return sum + Math.max(0, Number(income.expectedAmount) || 0);
    }, 0);
    var rolloverRate = Math.max(0, Number(data.controlAssumptions && data.controlAssumptions.creditCardRolloverCostRateMonthly) || 0);
    var rolloverCost = Math.round(revolving * rolloverRate / 100);
    monthlyDebtService += rolloverCost;
    var fundedReceivables = contracts.reduce(function (sum, contract) {
      if (!contract || contract.type !== 'receivable' || contract.status === 'closed') return sum;
      var linked = contract.fundingSource === 'borrowed' || contract.fundingSource === 'mixed' || (contract.fundingAllocations || []).some(function (row) { return row && row.sourceType === 'borrowed'; });
      return linked ? sum + Math.max(0, contract.currentOutstanding !== undefined && contract.currentOutstanding !== null ? Number(contract.currentOutstanding) || 0 : Number(contract.originalPrincipal) || 0) : sum;
    }, 0);
    return {
      cashBalance: total.liquid, totalAssets: total.assets, totalReceivables: total.receivable, totalDebt: total.liability,
      revolvingDebt: revolving, installmentDebt: installment,
      monthlyDebtService: monthlyDebtService, monthlyInterestExpense: monthlyInterest, monthlyFees: monthlyFees, unallocatedFees: unallocatedFees,
      expectedMonthlyIncome: expectedIncome, expectedLendingInterestIncome: lendingInterest,
      rolloverCostExposure: rolloverCost, netExpectedMonthlyCashflow: expectedIncome + lendingInterest - monthlyDebtService - plannedExpenses,
      fundingLinkedReceivables: fundedReceivables,
      liquidityByDate: projectionPath(accounts, flows, { baseDate: baseDate, horizonDays: Number(data.settings && data.settings.horizonDays) || 90 }, 'expected')
    };
  }
  ...
  function validateFlow(f, accMap) {
    ...
    if (f.kind === 'repay' || f.kind === 'collect') {
      var decomposed = f.kind === 'repay' ? repayTotal(f) : collectTotal(f);
      if (Math.abs(decomposed - Math.abs(Number(f.amount) || 0)) > 1) return 'Tổng tiền phải bằng gốc + lãi + phí.';
    }
  ...
  global.RootflowDomain = {
    ...
    repayPrincipal: repayPrincipal, repayInterest: repayInterest, repayFee: repayFee, repayCost: repayCost, repayTotal: repayTotal,
    collectPrincipal: collectPrincipal, collectInterest: collectInterest, collectFee: collectFee, collectTotal: collectTotal,
    contractPaymentTotals: contractPaymentTotals, contractOutstandingPrincipal: contractOutstandingPrincipal,
    contractSchedule: contractSchedule, contractInterestTotal: contractInterestTotal, contractFeeTotal: contractFeeTotal,
    ...
    forecast: forecast, monthSummary: monthSummary, controlMetrics: controlMetrics, overdue: overdue, upcoming: upcoming,
