...
(function (global) {
  ...
  function icon(name) {
    var paths = {
      ...
      settings: '<path d="M4 7h10m4 0h2M14 7a2 2 0 1 0 4 0 2 2 0 0 0-4 0ZM4 17h2m4 0h10M6 17a2 2 0 1 0 4 0 2 2 0 0 0-4 0Z"/>',
  ...
  function EventRow(props) {
    ...
    var neutral = event.kind === 'transfer' || amount === 0;
    var editable = typeof props.onEdit === 'function';
    var tag = editable ? 'button' : 'div';
    return h(tag, { type: editable ? 'button' : undefined, className: cx('rf-event-row', editable && 'rf-editable-row'), onClick: editable ? props.onEdit : undefined },
      h('time', { dateTime: event.date || '' }, fmtDate(event.date, false)),
      h('div', { className: 'rf-event-copy' }, h('strong', null, eventLabel(event)), h('div', { className: 'rf-event-meta' }, h('span', { className: cx('rf-confidence', confidence.cls) }, confidence.label), editable ? h('span', { className: 'rf-edit-hint' }, icon('edit'), 'Sửa') : null)),
      h('b', { className: neutral ? '' : amount < 0 ? 'negative' : 'positive' }, money(amount, !neutral)));
  }

  function recentEvents(data) {
    var accounts = D.byId(data.accounts || []);
    return (data.flows || []).filter(function (flow) {
      return flow && !flow.deletedAt && !flow.autoGenerated;
    }).sort(function (a, b) {
      return String(b.date || '').localeCompare(String(a.date || '')) || String(b.updatedAt || b.createdAt || '').localeCompare(String(a.updatedAt || a.createdAt || ''));
    }).map(function (flow) {
      var delta = D.liquidDelta(flow, accounts);
      return {
        id: flow.id,
        date: flow.date,
        amount: flow.kind === 'transfer' && !delta ? Math.abs(Number(flow.amount) || 0) : delta,
        kind: flow.kind,
        name: flow.counterpartyName || flow.title || flow.category || flow.note,
  ...
  function HomeScreen(props) {
    ...
    return h('div', { className: 'rf-os-screen rf-home' },
      ...
      h('section', { className: 'rf-home-plan-grid' },
        ...
        h('div', { className: 'rf-card rf-compact-card' }, h('div', { className: 'rf-card-icon' }, icon('capital')), h('span', null, 'Vốn có thể triển khai'), h('strong', null, money(op.deployableCapital)), h('small', null, 'Không dùng vào mức cần giữ'))));
  }
  ...
  function CashflowScreen(props) {
    ...
    var historyState = React.useState(false), showAllHistory = historyState[0], setShowAllHistory = historyState[1];
    var recordedAll = recentEvents(props.data);
    var recorded = showAllHistory ? recordedAll : recordedAll.slice(0, 12);
    return h('div', { className: 'rf-os-screen rf-cashflow' },
      ...
      h('section', { className: 'rf-card' }, h(SectionHead, { title: 'Dòng tiền theo thời gian' }), h('div', { className: 'rf-event-list' }, events.length ? events.map(function (event, i) { return h(EventRow, { event: event, onEdit: !event.synthetic && props.onEditFlow ? function () { props.onEditFlow(event.id); } : null, key: event.id || event.date + '-' + i }); }) : h('p', { className: 'rf-empty' }, 'Chưa có dòng tiền tương lai trong khoảng này.'))),
      recorded.length ? h('section', { className: 'rf-card' }, h(SectionHead, { title: 'Giao dịch đã nhập', action: recordedAll.length > 12 ? (showAllHistory ? 'Thu gọn' : 'Xem tất cả') : '', onAction: function () { setShowAllHistory(!showAllHistory); } }), h('p', { className: 'rf-section-note' }, 'Chạm vào giao dịch để sửa thông tin đã nhập.'), h('div', { className: 'rf-event-list' }, recorded.map(function (event) { return h(EventRow, { event: event, onEdit: props.onEditFlow ? function () { props.onEditFlow(event.id); } : null, key: event.id }); }))) : null);
  }
  ...
  function PositionRow(props) {
    var row = props.row;
    var editable = !!(row.accountId && props.onEditAccount);
    var tag = editable ? 'button' : 'div';
    return h(tag, { type: editable ? 'button' : undefined, className: cx('rf-position-row', editable && 'rf-editable-row'), onClick: editable ? function () { props.onEditAccount(row.accountId); } : undefined },
      ...
      h('div', { className: 'rf-position-value' }, h('strong', null, money(row.value)), row.recurringIncome ? h('span', null, '+' + compactMoney(row.recurringIncome) + '/th') : null, editable ? h('small', { className: 'rf-edit-hint' }, icon('edit'), 'Sửa') : null));
  }
  ...
  function FundingRow(props) {
    ...
    var editable = !!(source.accountId && props.onEditAccount);
    var tag = editable ? 'button' : 'div';
    return h(tag, { type: editable ? 'button' : undefined, className: cx('rf-funding-row', editable && 'rf-editable-row'), onClick: editable ? function () { props.onEditAccount(source.accountId); } : undefined }, h('div', { className: cx('rf-funding-icon', source.kind) }, icon(iconName)), h('div', { className: 'rf-funding-copy' }, h('div', null, h('strong', null, source.name), h('span', null, sourceKindLabel(source.kind))), h('small', null, meta.join(' · ') || 'Chưa đủ dữ liệu kỳ hạn')), h('div', { className: 'rf-funding-value' }, h('strong', null, money(source.balance)), h('span', null, 'Dư nợ'), editable ? h('small', { className: 'rf-edit-hint' }, icon('edit'), 'Sửa') : null));
  }
  function CapitalScreen(props) {
    ...
    if (props.mode === 'funding') {
      return h('div', { className: 'rf-os-screen rf-capital' },
        ...
        h('section', { className: 'rf-card' }, h(SectionHead, { title: 'Nguồn vốn đang sử dụng', action: 'Thêm', onAction: props.onAdd }), h('div', { className: 'rf-funding-list' }, cap.fundingSources.length ? cap.fundingSources.map(function (source) { return h(FundingRow, { source: source, onEditAccount: props.onEditAccount, key: source.id }); }) : h('p', { className: 'rf-empty' }, 'Chưa có nguồn vốn vay đang hoạt động.'))),
    ...
    return h('div', { className: 'rf-os-screen rf-capital' },
      ...
      h('section', { className: 'rf-card' }, h(SectionHead, { title: 'Vốn đang chạy', action: 'Thêm', onAction: props.onAdd }), h('div', { className: 'rf-position-list' }, cap.positions.length ? cap.positions.map(function (row) { return h(PositionRow, { row: row, onEditAccount: props.onEditAccount, key: row.id }); }) : h('p', { className: 'rf-empty' }, 'Chưa có khoản cho vay, đầu tư, kinh doanh hoặc tài sản đang theo dõi.'))),
  ...
  function PlanScreen(props) {
    ...
    return h('div', { className: 'rf-os-screen rf-plan' },
      h('div', { className: 'rf-plan-intro' }, h('h1', null, 'Kế hoạch'), h('p', null, 'Điều chỉnh giả định và xem ngay tác động lên số tiền có thể sử dụng.')),
      h(PlanCard, { iconName: 'salary', title: 'Thu nhập', value: money(income.plannedAmount), onEdit: function () { props.onEdit('salary'); }, rows: [['Mặc định', money(income.defaultAmount)], ['Ngày nhận', income.paymentDay ? String(income.paymentDay) : 'Chưa đặt'], [monthLabel(income.month), money(income.plannedAmount) + (income.hasOverride ? ' · điều chỉnh riêng' : '')]] }),
