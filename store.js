  ...
  function migrate(raw) {
    ...
    data.contracts = data.contracts.map(function (c) {
      ...
      if (!contract.settlementAccountId) {
        ...
        contract.settlementAccountId = linkedFlow ? linkedFlow.accountId : '';
      }
    ...
    data.flows.forEach(function (f) {
      ...
      if (f.autoGenerated === undefined) {
        var note = String(f.note || '');
        if (/^(Đáo hạn khoản vay|Dự kiến thu gốc và lãi|Thanh toán lãi theo lịch|Trả gốc cuối kỳ)$/.test(note)) {
          f.autoGenerated = true;
        }
      }
      if (f.autoGenerated && !f.scheduleRole) {
        f.scheduleRole = /^Thanh toán lãi/.test(String(f.note || '')) ? 'interest'
          : /^Trả gốc cuối kỳ/.test(String(f.note || '')) ? 'principal'
          : 'maturity';
      }
    });
